...
/* jenner-check: caller matching this macro's own docblock usage example
   (%util_getDsIdValues(p_apiBaseUrl=http://api.census.gov/data/1986/cbp
   , p_rtrnRowIDMacVarName=g_rowId, p_rtrnUniqueIDMacVarName=g_uniqueId)),
   run against a small mock registry table (built in autoexec.sas) shaped
   like the real APILIB._API_ALL_DATA table that censusapi_getAllDataSets.sas
   produces -- carrying just the columns this macro reads: _ROWID_,
   ds_unique_id, BaseURL. */
%GLOBAL g_dsRowId g_dsUniqueId;

%util_getDsIdValues(p_registryDsName=work._api_all_data
, p_apiBaseUrl=http://api.census.gov/data/2000/dec/sf1/
, p_rtrnRowIDMacVarName=g_dsRowId
, p_rtrnUniqueIDMacVarName=g_dsUniqueId)

data _null_;
  put "Resolved dataset row: _ROWID_=&g_dsRowId  ds_unique_id=&g_dsUniqueId";
run;
