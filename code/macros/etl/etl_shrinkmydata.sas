...
/**
 ******************************************************
 * Shrinks the input data set by reducing the variables
 * lengths to fit the largest value found in all the
 * records.
 * <br><br>Usage Example:<br>
 * %etl_shrinkMyData(p_inDsName=SASHELP.PRDSALE
 *						, p_outDsName=WORK.TEST
 *						, p_noCompress=);<br>
 *
 * @param p_inDsName	Two level data set name to
 *						be shrinked
 * @param p_outDsName	The name of the shrunk output
 *						table
 * @param p_noCompress	List of variables to be excluded
 *						from shrinking, Or it could be
 *						_character_/_numeric_ Name List
 * @param p_switchLimit Number of records controlling
 *						which Method to use for calculating
 ...
 *----------+----------+-----------------------------
 * alatt001 |05/11/2012|Initial creation
 * alatt001 |04/28/2017|Added support for _character_/_numeric_
 *          |          |Special SAS Name Lists
 * alatt001 |07/07/2020|Modified the code logic to utilize
 *          |          |Proc Append
 * alatt001 |10/15/2020|Modified the code logic to preserve
 *          |          |exiting variable format (formatl.formatd)
 * alatt001 |12/14/2020|Modified the code logic to
 *          |          |- preserve the attributes of the
 *          |          |excluded column(s)
 *          |          |- Reduce the minimum numeric
 *          |          |variable length to 3 instead of 4
 * alatt001 |04/04/2022|Modified the code logic to
 *          |          |- preserve the label attribute
 *          |          |- Replace %sysfunc(compress()) with the use of
 *          |          |TRIMMED
 * alatt001 |04/07/2022|Modified the code logic to
 *          |          |- Add a step to find the number of records in the
 *          |          |input data set
 *          |          |- Switch the method/query variables Max Legnth
 *          |          |values are computed, based on size of the data
 *          |          |(>= 5000000 records)
 *          |          |- Declare inner macros to generate required SAS
 *          |          |code statements used in the Max Length query and
 *          |          |data step.
 *          |          |- Use Hash Objects and Hash Iterators to store and
 *          |          |retrieve variable attributes instead of using long
 *          |          |string/character variables.
 * alatt001 |04/18/2022|Fixed the lblStr value, by using COALESCEC(LABEL,NAME)
 *          |          |to cater for cases when a lable is not defined for a 
 *          |          |variable.
 * alatt001 |04/26/2022|Added new parameter (p_switchLimit=) to control/switch 
 *          |          |the method used to calculate variables MAx length. 
 *          |          |
 *          |          |
 *          |          |
 *          |          |
...
%macro etl_shrinkMyData(
	...
	%if (%superq(p_noCompress) EQ ) %then
		%let l_noCompress =;
	...
	*/
	...
	DATA _NULL_;
		...
		*/
		...
		lblStr = CATS(NAME,"='",COALESCEC(LABEL,NAME),"'");
		rc = lblh.add();
		...
		do;
			...
			%if (%upcase(&p_inDsName) NE %upcase(&p_outDsName)) %then
			%do;
				PUT +3 "PROC APPEND BASE=&p_outDsName DATA=&p_inDsName FORCE NOWARN;";
				PUT +3 'RUN;';
			%end;
